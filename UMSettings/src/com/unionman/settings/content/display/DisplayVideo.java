...
public class DisplayVideo extends RightWindowBase {
	private HiDisplayManager display_manager;
	private CheckRadioButton crb_compare;
	private CheckRadioButton crb_mode;

	private static final String TAG = "com.unionman.settings.content.display--DisplayVideo";

	public DisplayVideo(Context paramContext) {
		super(paramContext);
	...
	public void setView() {
		...
		crb_compare.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CheckRadioButton paramCheckRadioButton,
				...
				try {
					DisplayVideo.this.layoutManager.showLayout(ConstantList.FRAME_DISPLAY_VIDEO_COMPARE);
		...
		crb_mode.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CheckRadioButton paramCheckRadioButton,
				...
				try {
					DisplayVideo.this.layoutManager.showLayout(ConstantList.FRAME_DISPLAY_VIDEO_MODE);
	...
	private void initModeOlderChorice() {
		...
		if(cvrs == 0){
			crb_mode.setText2("拉伸");
		}
		if(cvrs == 1){
			crb_mode.setText2("加黑边");
		}
