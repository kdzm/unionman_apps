...
static jint android_dvb_ProgManage_getDvbProgList(JNIEnv *env, jobject thiz, jobject proglist)
{
	...
	if(UM_SUCCESS == ret)
	{
        ...
        for (UM_U32 i = 0; i < plist.u32count; i++)
        {
            ...
            transType = env->GetFieldID(ProgClazz,"transType","I");
            logicNum = env->GetFieldID(ProgClazz,"logicNum","I");
            ...
            env->SetIntField(progObject, transType,(jint)prog_info->trans_type);
            env->SetIntField(progObject, logicNum, (jint)prog_info->u32logic_num);
            ...
            LOGV("bouquet_cnt =%d", plist.bouquet_cnt);
	        if (plist.bouquet_cnt > 0)
	        {
	            bouquetNamesFiled = env->GetFieldID(ProgClazz, "bouquetNames", "[Ljava/lang/String;");
	            bouquetIDFiled = env->GetFieldID(ProgClazz, "bouquetIds", "[I");
                jclass objClass = env->FindClass("java/lang/String");
                jobjectArray bouquets = env->NewObjectArray(plist.bouquet_cnt, objClass, 0);
                jintArray booquetIds = env->NewIntArray(plist.bouquet_cnt);
                jint *arrBooquetIds = (env)->GetIntArrayElements(booquetIds, NULL);
                ...
                for (int j=0; j<plist.bouquet_cnt; j++)
                {
                    ...
                    if (jstr != NULL) {
                    	...
                    	arrBooquetIds[j] = plist.bouquet_id[j];
                    }

                }
                (env)->ReleaseIntArrayElements(booquetIds, arrBooquetIds, 0);
			    if (bouquets != NULL) {
			    	env->SetObjectField(progObject, bouquetNamesFiled, bouquets);
					env->DeleteLocalRef(bouquets);
                }
			    if (booquetIds != NULL) {
			    	env->SetObjectField(progObject, bouquetIDFiled, booquetIds);
			    	env->DeleteLocalRef(booquetIds);
                }
			}
...
static jint android_dvb_ProgManage_createTestProgList(JNIEnv *env, jobject thiz, jint type)
{
    LOGV("android_dvb_ProgManage_createTestProgList \n");
    UM_S32 ret;
	sp<ProgManage> pm = getProgManage(env, thiz);
	ret =  pm->createTestProgList(type);
	LOGV("android_dvb_ProgManage_createTestProgList finished!\n");
	return ret;
}
static jint android_dvb_ProgManage_resetTestProgList(JNIEnv *env, jobject thiz)
{
    LOGV("android_dvb_ProgManage_resetTestProgList \n");
    UM_S32 ret;
	sp<ProgManage> pm = getProgManage(env, thiz);
	ret =  pm->resetTestProgList();
	LOGV("android_dvb_ProgManage_resetTestProgList finished!\n");
	return ret;
}
// ----------------------------------------------------------------------------
...
static JNINativeMethod gMethods[] = {
	...
	{"createTestProgList",                 "(I)I",                 (void *)android_dvb_ProgManage_createTestProgList},
    {"resetTestProgList",                 "()I",                 (void *)android_dvb_ProgManage_resetTestProgList},
};
