...
package com.um.gallery3d.photoeditor.actions;
...
class CropView extends FullscreenToolView {
    ...
    private void moveEdges(float deltaX, float deltaY) {
        ...
        if (movingEdges == MOVE_BLOCK) {
            ...
            deltaX = (deltaX > 0) ? Math.min(displayBounds.right - cropped.right, deltaX) : Math
                    .max(displayBounds.left - cropped.left, deltaX);
            deltaY = (deltaY > 0) ? Math.min(displayBounds.bottom - cropped.bottom, deltaY) : Math
                    .max(displayBounds.top - cropped.top, deltaY);
    ...
    private synchronized void drawShadow(Canvas canvas, float left, float top, float right, float bottom) {
        canvas.save();
