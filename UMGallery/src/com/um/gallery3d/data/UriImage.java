...
public class UriImage extends MediaItem {
    ...
    private void openFileOrDownloadTempFile(JobContext jc) {
        /*
         * int state = openOrDownloadInner(jc); synchronized (this) { mState =
         * state; if (mState != STATE_DOWNLOADED) { if (mFileDescriptor != null)
         * { Utils.closeSilently(mFileDescriptor); mFileDescriptor = null; } }
         * notifyAll(); }
         */
    }
    ...
    private ParcelFileDescriptor openOrDownloadInner(JobContext jc) {
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(scheme)
            ...
            try {
                ...
                if (mFileDescriptor.getStatSize() > MAX_FILE_SIZE) {
                    Log.i(TAG,
                            "This File is to large,so we needn't to decode it,and the file size is:"
                                    + mFileDescriptor.getStatSize() + " the max size of File is:"
                                    + MAX_FILE_SIZE);
                    return null;
                }
    ...
    private class RegionDecoderJob implements Job<BitmapRegionDecoder> {
        public BitmapRegionDecoder run(JobContext jc) {
            ...
            BitmapRegionDecoder decoder = DecodeUtils.requestCreateBitmapRegionDecoder(jc,
    ...
    private class BitmapJob implements Job<Bitmap> {
        ...
        public Bitmap run(JobContext jc) {
            ...
            if ((mContentType.toLowerCase().equals("image/giff") && mContentType.toLowerCase()
                    .endsWith("giff"))
                    || (mContentType.toLowerCase().equals("image/gif") && mContentType
                            .toLowerCase().endsWith("gif"))) {
                bitmap = DecodeUtils.decodeThumbnail(jc, mFileDescriptor.getFileDescriptor(), options, targetSize, mType);
            } else {
                Log.i(TAG, "+++++++++++++++++++ decodeFile " + "mType = " + mType);
                if (mType == MediaItem.TYPE_THUMBNAIL || mType == MediaItem.TYPE_SCALE) {
                   bitmap = DecodeUtils.requestDecodeFile(jc, getTiffPath(), options, targetSize);
                }
                else {
                   bitmap = DecodeUtils.decodeThumbnail(jc, mFileDescriptor.getFileDescriptor(), options, targetSize, mType);
                }
    ...
    private String getTiffPath() {
        ...
        try {
            cursor = mApplication.getContentResolver().query(
                    Uri.parse(uri.substring(0, uri.lastIndexOf("/"))), new String[] {
                        "_data"
                    }, "_id=" + uri.substring(uri.lastIndexOf("/") + 1), null, null);
