...
public class FilterSet extends MediaSet implements ContentListener {
    ...
    public FilterSet(Path path, DataManager dataManager, MediaSet baseSet, int mediaType) {
    ...
    public ArrayList<MediaItem> getMediaItem(int start, int count) {
        return ClusterAlbum.getMediaItemFromPath(mPaths, start, count, mDataManager);
    }

    @Override
    public long reload() {
     	Log.i(TAG, TAG+"reload============");
//        if (mBaseSet.reload() > mDataVersion) {
        	Log.i(TAG, TAG+"mBaseSet.reload() > mDataVersion");
            updateData();
