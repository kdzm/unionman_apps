...
import com.um.gallery3d.app.GalleryApp;
import com.um.gallery3d.common.Utils;
import com.um.gallery3d.list.ImageModel;
...
public class LocalAlbum extends MediaSet {
    ...
    public LocalAlbum(Path path, GalleryApp application, int bucketId, boolean isImage) {
        this(path, application, bucketId, isImage, LocalAlbumSet.getBucketName(
    ...
    public ArrayList<MediaItem> getMediaItem(int start, int count) {
        ...
        List<ImageModel> imageModeList = null;
        imageModeList = mApplication.getMediaFileListService().getList();
        ...
        if (imageModeList != null && imageModeList.size() > 0) {
            for (int i = 0; i < imageModeList.size(); i++) {
                if (i >= start && i <= (count + start)) {
                    ImageModel imageModel = imageModeList.get(i);
                    DataManager dm = mApplication.getDataManager();
                    Path itemPath = dm.findPathByUri(Uri.parse(imageModel.getPath()));
                    MediaItem mediaItem = (MediaItem) dm.getMediaObject(itemPath);
                    list.add(mediaItem);
                }
            }
        }
    ...
    public static MediaItem[] getMediaItemById(GalleryApp application, boolean isImage,
        ...
        Cursor cursor = resolver.query(baseUri, projection, "_id BETWEEN ? AND ?", new String[] {
        ...
        try {
            ...
            while (i < n && cursor.moveToNext()) {
                ...
                MediaItem item = loadOrUpdateItem(childPath, cursor, dataManager, application,
                        isImage);
                result[i] = item;
                ++i;
            }
    ...
    public static Cursor getItemCursor(ContentResolver resolver, Uri uri, String[] projection,
            int id) {
        return resolver.query(uri, projection, "_id=?", new String[] {
            String.valueOf(id)
        }, null);
    }
    ...
    public int getMediaItemCount() {
        ...
        mCachedCount = mApplication.getMediaFileListService().getSize();
    ...
    public void delete() {
        GalleryUtils.assertNotInRenderThread();
        // delete sdcard file
        Cursor cursor = mApplication.getContentResolver().query(mBaseUri, new String[] {
            ImageColumns.DATA
        ...
        try {
            while (cursor != null && cursor.moveToNext()) {
                String delFilePath = cursor.getString(0);

                if (delFilePath != null && !delFilePath.equals("")) {
                    isDel = new File(delFilePath).delete();

                    if (!isDel) {
                        Log.e(TAG, "delete file :" + delFilePath + " fail");
                        break;
                    } else {
                        Log.d(TAG, "delete file :" + delFilePath + " sucess");
                    }
                }
            }
        } catch (Exception e) {
        ...
        // end
        mResolver.delete(mBaseUri, mWhereClause, new String[] {
            String.valueOf(mBucketId)
        });
    }
