...
public class FilterUtils {
    ...
    public static void setupMenuItems(GalleryActionBar model, Path path, boolean inAlbum) {
        ...
        setMenuItemApplied(model, CLUSTER_BY_TIME, (ctype & CLUSTER_BY_TIME) != 0,
                (ccurrent & CLUSTER_BY_TIME) != 0);
        setMenuItemApplied(model, CLUSTER_BY_LOCATION, (ctype & CLUSTER_BY_LOCATION) != 0,
                (ccurrent & CLUSTER_BY_LOCATION) != 0);
        setMenuItemApplied(model, CLUSTER_BY_TAG, (ctype & CLUSTER_BY_TAG) != 0,
                (ccurrent & CLUSTER_BY_TAG) != 0);
        setMenuItemApplied(model, CLUSTER_BY_FACE, (ctype & CLUSTER_BY_FACE) != 0,
                (ccurrent & CLUSTER_BY_FACE) != 0);
        model.setClusterItemVisibility(CLUSTER_BY_ALBUM, !inAlbum || ctype == 0);
        setMenuItemApplied(model, R.id.action_cluster_album, ctype == 0, ccurrent == 0);
        ...
        setMenuItemAppliedEnabled(model, R.string.show_images_only,
                (ftype & FILTER_IMAGE_ONLY) != 0, (ftype & FILTER_IMAGE_ONLY) == 0 && ftypef == 0,
                (fcurrent & FILTER_IMAGE_ONLY) != 0);
        setMenuItemAppliedEnabled(model, R.string.show_videos_only,
                (ftype & FILTER_VIDEO_ONLY) != 0, (ftype & FILTER_VIDEO_ONLY) == 0 && ftypef == 0,
                (fcurrent & FILTER_VIDEO_ONLY) != 0);
        setMenuItemAppliedEnabled(model, R.string.show_all, ftype == 0, ftype != 0 && ftypef == 0,
                fcurrent == 0);
    }
    ...
    private static void setMenuItemApplied(GalleryActionBar model, int id, boolean applied,
            boolean updateTitle) {
        model.setClusterItemEnabled(id, !applied);
    }

    private static void setMenuItemAppliedEnabled(GalleryActionBar model, int id, boolean applied,
            boolean enabled, boolean updateTitle) {
