...
import com.um.gallery3d.common.BitmapUtils.RegionBitmap;
import com.um.gallery3d.data.ContentListener;
import com.um.gallery3d.data.DataManager;
import com.um.gallery3d.data.MediaItem;
import com.um.gallery3d.data.MediaObject;
import com.um.gallery3d.data.MediaSet;
import com.um.gallery3d.data.Path;
import com.um.gallery3d.list.Common;
import com.um.gallery3d.ui.GLView;
import com.um.gallery3d.ui.PhotoView;
import com.um.gallery3d.ui.SynchronizedHandler;
import com.um.gallery3d.ui.TileImageViewAdapter;
import com.um.gallery3d.ui.PhotoView.ImageData;
import com.um.gallery3d.util.Future;
import com.um.gallery3d.util.FutureListener;
import com.um.gallery3d.util.ThreadPool;
import com.um.gallery3d.util.ThreadPool.Job;
...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    private static final int DATA_CACHE_SIZE = 3;
    ...
    private boolean readToGetScale = true;
    ...
    /*
     * The ImageCache is a version-to-ImageEntry map. It only holds the
    ...
    private HashMap<Long, Bitmap> scaleBitmap = new HashMap<Long, Bitmap>();
    ...
    public PhotoDataAdapter(GalleryActivity activity, PhotoView view, MediaSet mediaSet,
            Path itemPath, int indexHint) {
    ...
    private void updateScreenNail(long version, Future<Bitmap> future) {
        ...
        if (entry.screenNail == null) {
            retryUpdateScreenNail(version, future);
        } else {
            if (mDataListener != null) {
    ...
    private void updateTileProvider(ImageEntry entry) {
        ...
        if (screenNail != null) {
            if (fullImage != null) {
                mTileProvider.setBackupImage(screenNail, fullImage.getWidth(),
                        fullImage.getHeight());
    ...
    private class RetryScreenNailListener implements Runnable, FutureListener<Bitmap> {
        private final long mVersion;

        private Future<Bitmap> mFuture;

        public RetryScreenNailListener(long version) {
            mVersion = version;
        }
        ...
        @Override
        public void run() {
            retryUpdateScreenNail(mVersion, mFuture);
        }
    }

    private void retryUpdateScreenNail(long version, Future<Bitmap> future) {
        ImageEntry entry = new ImageEntry();
        entry.screenNail = future.get();
        entry.failToLoad = false;
        if (mData[mCurrentIndex % DATA_CACHE_SIZE] != null) {
            entry.rotation = mData[mCurrentIndex % DATA_CACHE_SIZE].getFullImageRotation();
        }
        if (entry.screenNail == null) {
            Log.i("BlackScreen",
                    "retry updateScreenNail,but the ScreenNail is null,so we retry it again:"
                            + retryCount);
            Iterator<ImageEntry> iterator = mImageCache.values().iterator();
            while (iterator.hasNext()) {
                ImageEntry en = iterator.next();
                if (!(en.screenNail == null || en.screenNail.isRecycled())) {
                    en.screenNail.recycle();
                }
            }
            mImageCache.clear();
            ...
            if (retryCount <= 5) {
                mThreadPool.submit(
                        new RetryScreenNailJob(mData[mCurrentIndex % DATA_CACHE_SIZE]),
                        new RetryScreenNailListener(mData[mCurrentIndex % DATA_CACHE_SIZE]
                                .getDataVersion()));
                retryCount++;
                return;
            } else {
                entry.failToLoad = true;
                mPhotoView.setmLoadingState(3);
            ...
            mTileProvider.setBackupImage(entry.screenNail, entry.screenNail.getWidth(),
    ...
    private static class RetryScreenNailJob implements Job<Bitmap> {
        private MediaItem mItem;

        public RetryScreenNailJob(MediaItem item) {
            mItem = item;
        }
    ...
    private void getScaleBitmap(int x, int y) {
        if(mData[mCurrentIndex%DATA_CACHE_SIZE]!=null) {
            mThreadPool.submit(new ScaleBitmapJob(mData[mCurrentIndex % DATA_CACHE_SIZE]),
                    new ScaleBitmapListener(getVersion(mCurrentIndex), x, y));
        }
    }

    private class ScaleBitmapListener implements Runnable, FutureListener<Bitmap> {
        private final long mVersion;
        ...
        public ScaleBitmapListener(long version, int x, int y) {
            mVersion = version;
            this.x = x;
            this.y = y;
        }
        ...
        public void onFutureDone(Future<Bitmap> future) {
            mFuture = future;
            scaleBitmap.clear();
            scaleBitmap.put(mVersion, future.get());
            if (!readToGetScale) {
                tuneToPP(x, y);
            }
        }
    ...
    private static class ScaleBitmapJob implements Job<Bitmap> {
        private MediaItem mItem;

        public ScaleBitmapJob(MediaItem item) {
            mItem = item;
        }

        public Bitmap run(JobContext jc) {
            Bitmap bitmap = mItem.requestImage(MediaItem.TYPE_SCALE).run(jc);
    ...
    private Future<?> startTaskIfNeeded(int index, int which) {
        ...
        /*
         * if (which == BIT_FULL_IMAGE && (entry.requestedBits & BIT_FULL_IMAGE)
         * == 0 && (item.getSupportedOperations() &
         * MediaItem.SUPPORT_FULL_IMAGE) != 0) { entry.requestedBits |=
         * BIT_FULL_IMAGE; entry.fullImageTask = mThreadPool.submit(
         * item.requestLargeImage(), new
         * FullImageListener(item.getDataVersion())); // request full image
         * return entry.fullImageTask; }
         */
        return null;
    }
        ...
        private int findIndexOfTarget(UpdateInfo info) {
            ...
            if (items != null) {
                for (int i = 0, n = items.size(); i < n; ++i) {
                    if (items.get(i).getPath() == info.target) {
    ...
    public boolean tuneToPP(int x, int y) {
        if (scaleBitmap.get(getVersion(mCurrentIndex)) == null) {
            getScaleBitmap(x, y);
            readToGetScale = false;
            mMainHandler.sendEmptyMessage(MSG_LOAD_START);
            return false;
        }
        ...
        if (scaleBitmap.get(getVersion(mCurrentIndex)).getWidth() > mPhotoView.getWidth()
                || scaleBitmap.get(getVersion(mCurrentIndex)).getHeight() > mPhotoView.getHeight()) {
            RegionBitmap region = BitmapUtils.getPointToPointBitmap(
                    scaleBitmap.get(getVersion(mCurrentIndex)), x, y, mPhotoView.getWidth(),
                    mPhotoView.getHeight());
            pointBitmap = region.getBitmap();
            mTileProvider.setBackupImage(pointBitmap, pointBitmap.getWidth(),
                    pointBitmap.getHeight());
            mPhotoView.notifyModelInvalidated();
            return region.isResult();
        } else {
    ...
    public void refresh() {
        mTileProvider.setBackupImage(mImageCache.get(getVersion(mCurrentIndex)).screenNail,
                mImageCache.get(getVersion(mCurrentIndex)).screenNail.getWidth(),
                mImageCache.get(getVersion(mCurrentIndex)).screenNail.getHeight());
        mPhotoView.notifyModelInvalidated();
    }

    @Override
    public void rotatePicture(int rotate) {
        ImageEntry entry = mImageCache.get(getVersion(mCurrentIndex));
        entry.rotation = rotate;
        mPhotoView.notifyImageInvalidated(0);
    }

    private void freeUnusedBitmap() {
        Iterator<Entry<Long, ImageEntry>> iter = mImageCache.entrySet().iterator();
        while (iter.hasNext()) {
            Entry<Long, ImageEntry> e = iter.next();
            ImageEntry value = (ImageEntry) e.getValue();
            if (value.rotation != 0) {
                value.rotation = 0;
            }
        }
        if (pointBitmap != null && !pointBitmap.isRecycled()) {
            pointBitmap.recycle();
        }
        retryCount = 0;
    }
