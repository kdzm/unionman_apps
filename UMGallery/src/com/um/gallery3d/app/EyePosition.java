...
public class EyePosition {
    ...
    /*
     * We assume the user is at the following position /|\ user's eye | /
     * -G(gravity) | / |_/ / |/_____\ -Y (-y direction of device) user angel
     */
    private void onAccelerometerChanged(float gx, float gy, float gz) {
        ...
        mX = Utils.clamp((x * USER_ANGEL_COS / glength + tx * USER_ANGEL_SIN / length)
                * mUserDistance, -mLimit, mLimit);
        mY = -Utils.clamp((y * USER_ANGEL_COS / glength + ty * USER_ANGEL_SIN / length)
                * mUserDistance, -mLimit, mLimit);
    ...
    private void onGyroscopeChanged(float gx, float gy, float gz) {
        ...
        mX = Utils.clamp((float) (mX + x * t / Math.hypot(mZ, mX)), -mLimit, mLimit)
                * GYROSCOPE_RESTORE_FACTOR;
        mY = Utils.clamp((float) (mY + y * t / Math.hypot(mZ, mY)), -mLimit, mLimit)
                * GYROSCOPE_RESTORE_FACTOR;
        mZ = (float) -Math.sqrt(mUserDistance * mUserDistance - mX * mX - mY * mY);
        mListener.onEyePositionChanged(mX, mY, mZ);
    }
    ...
    public void resume() {
        if (mSensor != null) {
            SensorManager sManager = (SensorManager) mContext
                    .getSystemService(Context.SENSOR_SERVICE);
            sManager.registerListener(mPositionListener, mSensor, SensorManager.SENSOR_DELAY_GAME);
        }
