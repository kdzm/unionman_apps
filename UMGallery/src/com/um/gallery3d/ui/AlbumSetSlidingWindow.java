...
public class AlbumSetSlidingWindow implements AlbumSetView.ModelListener {
    ...
    public AlbumSetSlidingWindow(GalleryActivity activity, AlbumSetView.LabelSpec labelSpec,
            SelectionDrawer drawer, AlbumSetView.Model source, int cacheSize) {
    ...
    public AlbumSetItem get(int slotIndex) {
        Utils.assertTrue(isActiveSlot(slotIndex), "invalid slot: %s outsides (%s, %s)", slotIndex,
    ...
    public void setActiveWindow(int start, int end) {
        Utils.assertTrue(start <= end && end - start <= mData.length && end <= mSize,
                "start = %s, end = %s, length = %s, size = %s", start, end, mData.length, mSize);
    ...
    private boolean isCoverItemsChanged(int slotIndex) {
        ...
        if (original.covers.length != coverItems.length) {
            return true;
        }
        ...
        for (int i = 0, n = coverItems.length; i < n; ++i) {
            ...
            if (g.mDataVersion != coverItems[i].getDataVersion()) {
                return true;
            }
        }
    ...
    private void notifySlotChanged(int slotIndex) {
        ...
        if (slotIndex < mContentStart || slotIndex >= mContentEnd) {
            Log.w(TAG, String.format("invalid update: %s is outside (%s, %s)", slotIndex,
    ...
    private void updateAllImageRequests() {
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            for (DisplayItem item : mData[i % mData.length].covers) {
                ...
                if (coverItem.isRequestInProgress()) {
                    ++mActiveRequestCount;
                }
            }
        }
    ...
    private class GalleryDisplayItem extends AbstractDisplayItem implements FutureListener<Bitmap> {
        ...
        public int render(GLCanvas canvas, int pass) {
            ...
            mSelectionDrawer.draw(canvas, mContent, width, height, getRotation(), path, sourceType,
                    mMediaType, mIsPanorama, mLabelSpec.labelBackgroundHeight,
                    cacheFlag == MediaSet.CACHE_FLAG_FULL, (cacheFlag == MediaSet.CACHE_FLAG_FULL)
        ...
        public void startLoadBitmap() {
            mFuture = mThreadPool.submit(mMediaItem.requestImage(MediaItem.TYPE_MICROTHUMBNAIL),
                    this);
        }
    ...
    private class LabelDisplayItem extends DisplayItem {
        ...
        public boolean updateContent() {
            ...
            if (Utils.equals(title, mTitle) && Utils.equals(count, mCount)
            ...
            mTextureTitle = StringTexture.newInstance(title, s.titleFontSize, FONT_COLOR_TITLE,
                    mBoxWidth - s.leftMargin, false);
            mTextureCount = StringTexture.newInstance(count, s.countFontSize, FONT_COLOR_COUNT,
