...
class PositionController {
    ...
    public void setImageSize(int width, int height) {
        ...
        float ratio = (float) mImageH / height;
    ...
    private void scrollBy(float dx, float dy, int type) {
        startAnimation(getTargetX() + Math.round(dx / mCurrentScale),
                getTargetY() + Math.round(dy / mCurrentScale), mCurrentScale, type);
    }

	private void setDialogContent(int textid, int showTime){
      final CustomToast myToast=new CustomToast(mContext);
    ...
    public void startScroll(float dx, float dy, boolean hasNext, boolean hasPrev) {
        ...
        Log.i(TAG, "!startScroll y"+y);
        Log.i(TAG, "!startScroll mBoundTop"+mBoundTop);
        Log.i(TAG, "!startScroll mBoundBottom"+mBoundBottom);
        ...
        if (!hasPrev && x < mBoundLeft) {
        	//Toast.makeText(mContext,mContext.getResources().getString(R.string.toast_firpic), Toast.LENGTH_SHORT).show();//leftedge
        	setDialogContent(R.string.toast_firpic, 3000);
        	...
        	//Toast.makeText(mContext,mContext.getResources().getString(R.string.toast_laspic), Toast.LENGTH_SHORT).show();//rightedge
        	setDialogContent(R.string.toast_laspic, 3000);
    ...
    public boolean fling(float velocityX, float velocityY) {
        ...
        calculateStableBound(mCurrentScale);
        mScroller.fling(mCurrentX, mCurrentY, Math.round(-velocityX / mCurrentScale),
                Math.round(-velocityY / mCurrentScale), mBoundLeft, mBoundRight, mBoundTop,
                mBoundBottom);
    ...
    private void startAnimation(int targetX, int targetY, float scale, int kind) {
        if (targetX == mCurrentX && targetY == mCurrentY && scale == mCurrentScale) {
            return;
        }
    ...
    private boolean useCurrentValueAsTarget() {
        return mAnimationStartTime == NO_ANIMATION || mAnimationKind == ANIM_KIND_SNAPBACK
    ...
    public RectF getImageBounds() {
        ...
        rect.set(Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY,
