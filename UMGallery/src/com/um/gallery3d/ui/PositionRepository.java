...
public class PositionRepository {
    ...
    public static class Position implements Cloneable {
        ...
        public boolean equals(Object object) {
            ...
            return x == position.x && y == position.y && z == position.z && theta == position.theta
                    && alpha == position.alpha;
        }

        public static void interpolate(Position source, Position target, Position output,
                float progress) {
            if (progress < 1f) {
                output.set(Utils.interpolateScale(source.x, target.x, progress),
