...
import com.um.gallery3d.util.IntArray;
...
public class GLCanvasImpl implements GLCanvas {
    ...
    private void drawMixed(BasicTexture from, int toColor, float ratio, int x, int y, int width,
        ...
        setTextureColor(((toColor >>> 16) & 0xff) * colorAlpha, ((toColor >>> 8) & 0xff)
                * colorAlpha, (toColor & 0xff) * colorAlpha, comboRatio);
    ...
    private void drawMixed(BasicTexture from, BasicTexture to, float ratio, int x, int y,
        ...
        mGLState.setBlendEnabled(mBlendEnabled
                && (!from.isOpaque() || !to.isOpaque() || alpha < OPAQUE_ALPHA));
        final GL11 gl = mGL;
    ...
    private static boolean isMatrixRotatedOrFlipped(float matrix[]) {
        ...
        return Math.abs(matrix[MSKEW_X]) > eps || Math.abs(matrix[MSKEW_Y]) > eps
                || matrix[MSCALE_X] < -eps || matrix[MSCALE_Y] > eps;
    }
    ...
    public BasicTexture copyTexture(int x, int y, int width, int height) {
        ...
        gl.glCopyTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGB, x, y, texture.getTextureWidth(),
    ...
    public void dumpStatisticsAndClear() {
        String line = String
                .format("MESH:%d, TEX_OES:%d, TEX_RECT:%d, FILL_RECT:%d, LINE:%d", mCountDrawMesh,
