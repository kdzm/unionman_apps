...
public class ScrollBarView extends GLView {
    ...
    public ScrollBarView(Context context, int gripHeight, int gripWidth) {
        ...
        context.getTheme()
                .resolveAttribute(android.R.attr.scrollbarThumbHorizontal, outValue, true);
    ...
    /*
     * @Override protected boolean onTouch(MotionEvent event) { switch
     * (event.getAction()) { case MotionEvent.ACTION_DOWN: { int x = (int)
     * event.getX(); return (x >= mGripPosition && x < mGripPosition +
     * mGripWidth); } case MotionEvent.ACTION_MOVE: { // Adjust x by mGripWidth
     * / 2 so the center of the grip // matches the touch position. int x =
     * (int) event.getX() - mGripWidth / 2; x = Utils.clamp(x, 0, getWidth() -
     * mGripWidth); if (mGripPosition != x) { mGripPosition = x;
     * notifyContentPositionFromGrip(); invalidate(); } break; } } return true;
     * }
     */
}
