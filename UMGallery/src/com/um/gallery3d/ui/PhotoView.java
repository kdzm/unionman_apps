...
import com.um.gallery3d.data.DecodeUtils;
import com.um.gallery3d.data.Path;
import com.um.gallery3d.list.Common;
import com.um.gallery3d.ui.PositionRepository.Position;
...
public class PhotoView extends GLView {
    ...
    private static final int MIN_TIME_INTERVAL_KEYS = 300;

    private static final int TIME_DELAY_MESSAGE_SEND = 300;
    ...
    public interface PhotoTapListener {
        ...
        public boolean actionSlideshow();
    ...
    private final static int LEFT_SCROLL_MSG = 998;
    private final static int RIGHT_SCROLL_MSG = 999;
    ...
    public PhotoView(GalleryActivity activity) {
        ...
        mNoThumbnailText = StringTexture.newInstance(context.getString(R.string.no_thumbnail),
                DEFAULT_TEXT_SIZE, Color.WHITE);
            ...
            public void handleMessage(Message message) {
                switch (message.what) {
                    ...
                    case MSG_REFRESH:
                        ...
                        if (retryCount <= 3) {
                        mTileView.retryNotifyImageInvalidated();
                        sendEmptyMessageDelayed(MSG_REFRESH, 1500);
                        }
                        */
                        //end
    ...
    public void setScale(float scale) {
        if (mTileView.setPosition(mPositionController.getImageWidth() / 2,
                mPositionController.getImageHeight() / 2, scale, 0)) {
            layoutScreenNails();
        }
    }

    private void updateScreenNailEntry(int which, ImageData data) {
        if (mTransitionMode == TRANS_SWITCH_NEXT || mTransitionMode == TRANS_SWITCH_PREVIOUS) {
    ...
    public void notifyImageInvalidated(int which) {
        switch (which) {
            ...
            case 0: {
                ...
                retryCount = 0;
                mHandler.sendEmptyMessageDelayed(MSG_REFRESH, 1500);
                mImageRotation = mModel.getImageRotation();

                if (((mImageRotation / 90) & 1) == 0) {
                    mPositionController.setImageSize(mTileView.mImageWidth, mTileView.mImageHeight);
                } else {
                    mPositionController.setImageSize(mTileView.mImageHeight, mTileView.mImageWidth);
                }
    ...
    private void updateLoadingState() {
        ...
        // COMPLETE --> INIT
        // FAIL --> INIT
        if (mModel.isFailedToLoad() && mLoadingState == LOADING_FAIL) {
            mHandler.removeMessages(MSG_SHOW_LOADING);
            mLoadingState = LOADING_FAIL;
        } else if (mModel.getLevelCount() != 0 || mModel.getBackupImage() != null) {
            mHandler.removeMessages(MSG_SHOW_LOADING);
            mLoadingState = LOADING_COMPLETE;
        } else if (mLoadingState != LOADING_INIT) {
            mLoadingState = LOADING_INIT;
            mHandler.removeMessages(MSG_SHOW_LOADING);
            mHandler.sendEmptyMessageDelayed(MSG_SHOW_LOADING, DELAY_SHOW_LOADING);
        }
    }
    ...
    /*
     * Here is how we layout the screen nails previous current next ___________
     * ________________ __________ | _______ | | __________ | | ______ | | | | |
     * | | right->| | | | | | | | |<-------->|<--left | | | | | | | |_______| |
     * | | |__________| | | |______| | |___________| | |________________|
     * |__________| | <--> gapToSide() | IMAGE_GAP + Max(previous.gapToSide(),
     * current.gapToSide)
     */
    private void layoutScreenNails() {
        ...
        if (entry.isEnabled()) {
            entry.layoutRightEdgeAt(left - (IMAGE_GAP + Math.max(gap, entry.gapToSide())));
        }
        ...
        if (entry.isEnabled()) {
            entry.layoutLeftEdgeAt(right + (IMAGE_GAP + Math.max(gap, entry.gapToSide())));
        }
    }
    ...
    protected void render(GLCanvas canvas) {
        ...
        if (mTransitionMode != TRANS_SLIDE_IN_LEFT && mTransitionMode != TRANS_SLIDE_IN_RIGHT
            ...
            boolean prevNailResult = prevNail == null || prevNail.mTexture == null
                    || prevNail.mTexture.mBitmap == null;
            boolean nextNailResult = nextNail == null || nextNail.mTexture == null
                    || nextNail.mTexture.mBitmap == null;

            if (prevNail.mVisible || !prevNailResult) {
                prevNail.draw(canvas);
            }
            if (nextNail.mVisible || !nextNailResult) {
                nextNail.draw(canvas);
            }
            */
            //end

            if (mTransitionMode == TRANS_SWITCH_PREVIOUS)
            {
                ScreenNailEntry prevNail = mScreenNails[ENTRY_PREVIOUS];
                ...
                if (prevNail.mVisible || !prevNailResult) {
                    ...
                    if (prevNail.mTexture != null) {
                        ...
                        if (prevNail.mTexture.mBitmap != null) {
                            prevNail.mTexture.mBitmap.recycle();
                            prevNail.mTexture.mBitmap = null;
                        }
                        prevNail.mTexture.recycle();
                        prevNail.mTexture = null;
                    }
                    //end
            ...
            else if (mTransitionMode == TRANS_SWITCH_NEXT)
            {
                ScreenNailEntry nextNail = mScreenNails[ENTRY_NEXT];
                ...
                if (nextNail.mVisible || !nextNailResult) {
                    ...
                    if (nextNail.mTexture != null) {
                        if (nextNail.mTexture.mBitmap != null) {
                            nextNail.mTexture.mBitmap.recycle();
                            nextNail.mTexture.mBitmap = null;
                        }
        ...
        if (mShowVideoPlayIcon && mLoadingState != LOADING_INIT && mLoadingState != LOADING_TIMEOUT) {
    ...
    private boolean swipeImages(float velocity) {
        if (mTransitionMode != TRANS_NONE && mTransitionMode != TRANS_SWITCH_NEXT
        ...
        if (velocity < -SWIPE_THRESHOLD && (isMinimal || controller.isAtRightEdge())) {
        ...
        } else if (velocity > SWIPE_THRESHOLD && (isMinimal || controller.isAtLeftEdge())) {
            stopCurrentSwipingIfNeeded();
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float dx, float dy) {
            ...
            ScreenNailEntry next = mScreenNails[ENTRY_NEXT];
            ScreenNailEntry prev = mScreenNails[ENTRY_PREVIOUS];
            mPositionController.startScroll(dx, dy, next.isEnabled(), prev.isEnabled());
            return true;
        }
        ...
        public boolean onDoubleTap(MotionEvent e) {
            ...
            if (scale <= 1.0f || controller.isAtMinimalScale()) {
                controller.zoomIn(e.getX(), e.getY(), Math.max(1.5f, scale * 1.5f));
            } else {
    ...
    private class MyScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (Float.isNaN(scale) || Float.isInfinite(scale) || mTransitionMode != TRANS_NONE) {
                return true;
            }

            mPositionController.scaleBy(scale, detector.getFocusX(), detector.getFocusY());
            return true;
        }
        ...
        public boolean onScaleBegin(ScaleGestureDetector detector) {
            ...
            mPositionController.beginScale(detector.getFocusX(), detector.getFocusY());
            return true;
        }
    ...
    private boolean switchToPPImage(int x, int y) {
        mTileView.invalidateTiles();
        return mModel.tuneToPP(x, y);
    }

    private void rotateImage(int rotate) {
        mTileView.invalidateTiles();
        mModel.rotatePicture(rotate);
    }
    ...
    public static interface Model extends TileImageView.Model {
        ...
        public boolean tuneToPP(int x, int y);

        public void rotatePicture(int x);
    ...
    private class ScreenNailEntry {
            ...
            if (bitmap == null) {
                ...
                if (mTexture != null) {
                    if (mTexture.getBitmap() != bitmap) {
                        ...
                        mTexture.mState = BasicTexture.STATE_UNLOADED;
                    }
                } else {
                    mTexture = new BitmapTexture(bitmap);
                }
        ...
        public int gapToSide() {
            return ((mRotation / 90) & 1) != 0 ? PhotoView.gapToSide(mDrawHeight, getWidth())
        ...
        public void updateDrawingSize() {
            ...
            float s = ((mRotation / 90) & 0x01) == 0 ? mPositionController.getMinimalScale(width,
                    height) : mPositionController.getMinimalScale(height, width);
        ...
        public synchronized void draw(GLCanvas canvas) {
            int x = mOffsetX;
            int y = getHeight() / 2;
            ...
            if (mTexture != null) {
                ...
                mTexture.draw(canvas, x - mDrawWidth / 2, y - mDrawHeight / 2, mDrawWidth,
                        mDrawHeight);
    ...
    public boolean getTime(long spaceTime) {
        long time = 0;
    ...
    Handler handler_flipping = new Handler() {
        public void handleMessage(android.os.Message msg) {
            if(msg.what == LEFT_SCROLL_MSG) {
                ScreenNailEntry next = mScreenNails[ENTRY_NEXT];
                ScreenNailEntry prev = mScreenNails[ENTRY_PREVIOUS];             
                		
                mPositionController.startScroll(-getWidth() / 3, 0.0f, next.isEnabled(), prev.isEnabled());
                mEdgeView.onRelease();
            ...
            else if(msg.what == RIGHT_SCROLL_MSG) {
                ScreenNailEntry next = mScreenNails[ENTRY_NEXT];
                ScreenNailEntry prev = mScreenNails[ENTRY_PREVIOUS];
    ...
    public class MyCustomKeyListener {
        public boolean onKey(GLRootView glRootView, int keyCode, KeyEvent event) {
            ...
            if (!GalleryUtils.isGLRootFocus && event.getKeyCode() != KeyEvent.KEYCODE_DPAD_DOWN) {
                return false;
            }
            switch (keyCode) {
                case KeyEvent.KEYCODE_DPAD_LEFT:
                    if (glRootView.getState() == PICTURE_STATE.NONE) {
                        if (!getTime(MIN_TIME_INTERVAL_KEYS)) {
                            return true;
                        }
                        
                        Common.imgCurrPos--;
                        if(Common.imgCurrPos<0){
                        	Common.imgCurrPos=0;
                        }
                        
                        Message msg = new Message();
                        msg.what = LEFT_SCROLL_MSG;
                        handler_flipping.sendMessageDelayed(msg, TIME_DELAY_MESSAGE_SEND);
                    } else if (glRootView.getState() == PICTURE_STATE.ZOOM) {
                        currentX--;
                        if (!switchToPPImage(currentX, currentY)) {
                            currentX++;
                        }
                        if (mUIListener != null) {
                            mUIListener.onUserInteractionEnd();
                        }
                    } else if (glRootView.getState() == PICTURE_STATE.ROTATE) {
                        rotate--;
                        if (rotate < 0) {
                            rotate += 4;
                        }
                        rotateImage((rotate % 4) * 90);
                    }
                ...
                case KeyEvent.KEYCODE_DPAD_RIGHT:
                    if (glRootView.getState() == PICTURE_STATE.NONE) {
                        ...
                        Common.imgCurrPos++;
                        if(Common.imgCurrPos>Common.imgListSize-1){
                        	Common.imgCurrPos=Common.imgListSize-1;
                        }
                        
                        
                        Message msg = new Message();
                        msg.what = RIGHT_SCROLL_MSG;
                        handler_flipping.sendMessageDelayed(msg, TIME_DELAY_MESSAGE_SEND);
                    } else if (glRootView.getState() == PICTURE_STATE.ZOOM) {
                        currentX++;
                        if (!switchToPPImage(currentX, currentY)) {
                ...
                case KeyEvent.KEYCODE_DPAD_CENTER:
                    ...
                    if (glRootView.getState() == PICTURE_STATE.NONE) {
                        if( mPhotoTapListener.actionSlideshow() )
                        {
                            glRootView.setState(PICTURE_STATE.SLIDE);
                        }
                    } else if (glRootView.getState() == PICTURE_STATE.ZOOM) {
                        currentY = 0;
                        currentX = 0;
                        switchToPPImage(currentX, currentY);
                    }
                    return true;

                case KeyEvent.KEYCODE_BACK:
                case KeyEvent.KEYCODE_ESCAPE:
                    if (glRootView.getState() == PICTURE_STATE.ROTATE) {
                        rotateImage(0);
                    } else if (glRootView.getState() == PICTURE_STATE.ZOOM) {
                        mModel.jumpTo(-1);
                    }
                    glRootView.setState(PICTURE_STATE.NONE);
                    return true;
            }
    ...
    public void setmLoadingState(int mLoadingState) {
