...
public class GLView {
    ...
    // The current get the focus slotView
    protected int mCurrentFocusSlotIndex = -1;

    // The current get the focus photoSlotView
    public static int mCurrrentPhotoSlotIndex = -1;

    public static enum PICTURE_STATE {
        NONE, ZOOM, ROTATE, SLIDE,
    }
    ...
    private void removeOneComponent(GLView component) {
        if (mMotionTarget == component) {
            ...
            MotionEvent cancelEvent = MotionEvent.obtain(now, now, MotionEvent.ACTION_CANCEL, 0, 0,
                    0);
            dispatchTouchEvent(null, cancelEvent);
            cancelEvent.recycle();
        }
    ...
    protected synchronized void renderChild(GLCanvas canvas, GLView component) {
    ...
    protected boolean dispatchTouchEvent(GLRootView glRootView, MotionEvent event, int x, int y,
        ...
        if (!checkBounds || rect.contains(x, y)) {
            ...
            if (component.dispatchTouchEvent(glRootView, event)) {
                event.offsetLocation(left, top);
                return true;
            }
    ...
    protected boolean dispatchTouchEvent(GLRootView glRootView, MotionEvent event) {
        ...
        if (mMotionTarget != null) {
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                dispatchTouchEvent(glRootView, cancel, x, y, mMotionTarget, false);
                mMotionTarget = null;
            } else {
                dispatchTouchEvent(glRootView, event, x, y, mMotionTarget, false);
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            for (int i = getComponentCount() - 1; i >= 0; --i) {
                ...
                if (dispatchTouchEvent(glRootView, event, x, y, component, true)) {
        ...
        return onTouch(glRootView, event);
    }

    // dispatch key to SlotView
    protected boolean dispatchKeyEvent(GLRootView glRootView, KeyEvent event) {
        int action = event.getAction();
        ...
        if (action == KeyEvent.ACTION_UP) {
            for (int i = getComponentCount() - 1; i >= 0; --i) {
                ...
                if (component instanceof GLView) {
                    mKeyTarget = component;
                    component.dispatchKeyEvent(glRootView, event);
                }
            }

            return onKeyDown(glRootView, event);
        }
