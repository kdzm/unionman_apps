...
import com.um.gallery3d.common.LruCache;
...
import com.um.gallery3d.util.JobLimiter;
...
public class AlbumSlidingWindow implements AlbumView.ModelListener {
    ...
    public static interface Listener {
        ...
        public void onWindowContentChanged(int slot, DisplayItem old, DisplayItem update);
    }
    ...
    public AlbumSlidingWindow(GalleryActivity activity, AlbumView.Model source, int cacheSize,
            int cacheThumbSize) {
    ...
    private void prepareSlotContent(final int slotIndex) {
        mData[slotIndex % mData.length] = new AlbumDisplayItem(slotIndex, mSource.get(slotIndex));
    }
    ...
    private class AlbumDisplayItem extends AbstractDisplayItem implements FutureListener<Bitmap>,
            Job<Bitmap> {
        ...
        public AlbumDisplayItem(int slotIndex, MediaItem item) {
            super(item);
            mMediaType = (item == null) ? MediaItem.MEDIA_TYPE_UNKNOWN : item.getMediaType();
        ...
        public int render(GLCanvas canvas, int pass) {
            ...
            if (pass == 0) {
                ...
                if (mMediaItem != null) {
                    path = mMediaItem.getPath();
                }

                mSelectionDrawer.draw(canvas, mContent, width, height, getRotation(), path,
                        mMediaType, mIsPanorama);
        ...
        public void startLoadBitmap() {
            if (mCacheThumbSize > 0) {
                ...
                mFuture = mThreadPool.submit(
                        mMediaItem.requestImage(MediaItem.TYPE_MICROTHUMBNAIL), this);
            }
        }

        // This gets the bitmap and scale it down.
        public Bitmap run(JobContext jc) {
            Job<Bitmap> job = mMediaItem.requestImage(MediaItem.TYPE_MICROTHUMBNAIL);
            Bitmap bitmap = job.run(jc);

            if (bitmap != null) {
                bitmap = BitmapUtils.resizeDownBySideLength(bitmap, mCacheThumbSize, true);
            }
