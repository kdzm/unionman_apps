...
import com.um.gallery3d.anim.Animation;
...
public class CropView extends GLView {
    ...
    public void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mImageHeight != SIZE_UNKNOWN) {
            ...
            if (mHighlightRectangle.getVisibility() == GLView.VISIBLE) {
                mAnimation.parkNow(mHighlightRectangle.mHighlightRect);
            }
        }
    }
    ...
    public void render(GLCanvas canvas) {
        ...
        if (a.calculate(canvas.currentAnimationTimeMillis())) {
            invalidate();
        }
    ...
    public RectF getCropRectangle() {
        if (mHighlightRectangle.getVisibility() == GLView.INVISIBLE) {
            return null;
        }

        RectF rect = mHighlightRectangle.mHighlightRect;
        RectF result = new RectF(rect.left * mImageWidth, rect.top * mImageHeight, rect.right
                * mImageWidth, rect.bottom * mImageHeight);
        return result;
    }
    ...
    private class AnimationController extends Animation {
        ...
        public void initialize() {
            ...
            mCurrentScale = Math.min(2,
                    Math.min((float) getWidth() / mImageWidth, (float) getHeight() / mImageHeight));
        }
        ...
        public void startParkingAnimation(RectF highlight) {
            ...
            if (wr >= MIN_SELECTION_RATIO && wr < MAX_SELECTION_RATIO && hr >= MIN_SELECTION_RATIO
                    && hr < MAX_SELECTION_RATIO && r.left >= d && r.right < width - d && r.top >= d
                    && r.bottom < height - d) {
                return;
            }
        ...
        public void inverseMapPoint(PointF point) {
            ...
            point.x = Utils.clamp(((point.x - getWidth() * 0.5f) / s + mCurrentX) / mImageWidth, 0,
                    1);
            point.y = Utils.clamp(((point.y - getHeight() * 0.5f) / s + mCurrentY) / mImageHeight,
                    0, 1);
        }
        ...
        public RectF mapRect(RectF input, RectF output) {
            ...
            int y = mCurrentY;
            float s = mCurrentScale;
            output.set(offsetX + (input.left * mImageWidth - x) * s, offsetY
                    + (input.top * mImageHeight - y) * s, offsetX + (input.right * mImageWidth - x)
                    * s, offsetY + (input.bottom * mImageHeight - y) * s);
            return output;
        }
        ...
        protected void onCalculate(float progress) {
            ...
            if (mCurrentX == mTargetX && mCurrentY == mTargetY && mCurrentScale == mTargetScale) {
                forceStop();
            }
        }
        ...
        private void calculateTarget(RectF highlight) {
            ...
            if (mImageWidth != SIZE_UNKNOWN) {
                ...
                float scale = Utils.clamp(
                        SELECTION_RATIO
                                * Math.min(width / (highlight.width() * mImageWidth), height
                                        / (highlight.height() * mImageHeight)), minScale, 2f);
                int centerX = Math.round(mImageWidth * (highlight.left + highlight.right) * 0.5f);
                int centerY = Math.round(mImageHeight * (highlight.top + highlight.bottom) * 0.5f);

                if (Math.round(mImageWidth * scale) > width) {
                    int limitX = Math.round(width * 0.5f / scale);
                    centerX = Math.round((highlight.left + highlight.right) * mImageWidth / 2);
                ...
                if (Math.round(mImageHeight * scale) > height) {
                    ...
                    centerY = Math.round((highlight.top + highlight.bottom) * mImageHeight / 2);
    ...
    private class HighlightRectangle extends GLView {
        ...
        public HighlightRectangle() {
            mArrow = new ResourceTexture(mActivity.getAndroidContext(), R.drawable.camera_crop_holo);
        }

        public void setInitRectangle() {
            float targetRatio = mAspectRatio == UNSPECIFIED ? 1f : mAspectRatio * mImageHeight
                    / mImageWidth;
        ...
        private void setMovingEdges(MotionEvent event) {
            ...
            if (inVerticalRange) {
                ...
                if (left) {
                    mMovingEdges |= MOVE_LEFT;
                }

                if (right) {
                    mMovingEdges |= MOVE_RIGHT;
                }

                if (mAspectRatio != UNSPECIFIED && inHorizontalRange) {
                    mMovingEdges |= (y > (r.top + r.bottom) / 2) ? MOVE_BOTTOM : MOVE_TOP;
                }
            }
            ...
            if (inHorizontalRange) {
                ...
                if (top) {
                    mMovingEdges |= MOVE_TOP;
                }

                if (bottom) {
                    mMovingEdges |= MOVE_BOTTOM;
                }

                if (mAspectRatio != UNSPECIFIED && inVerticalRange) {
                    mMovingEdges |= (x > (r.left + r.right) / 2) ? MOVE_RIGHT : MOVE_LEFT;
                }
            }
        }
        ...
        protected void renderBackground(GLCanvas canvas) {
            ...
            if ((mMovingEdges & MOVE_RIGHT) != 0 || notMoving) {
                mArrow.draw(canvas, Math.round(r.right - mArrow.getWidth() / 2),
                        Math.round(centerY - mArrow.getHeight() / 2));
            }

            if ((mMovingEdges & MOVE_LEFT) != 0 || notMoving) {
                mArrow.draw(canvas, Math.round(r.left - mArrow.getWidth() / 2),
                        Math.round(centerY - mArrow.getHeight() / 2));
            }

            if ((mMovingEdges & MOVE_TOP) != 0 || notMoving) {
                mArrow.draw(canvas, Math.round(centerX - mArrow.getWidth() / 2),
    ...
    private class DetectFaceTask extends Thread {
        ...
        public void run() {
            ...
            mMainHandler.sendMessage(mMainHandler.obtainMessage(MSG_UPDATE_FACES, this));
        }
        ...
        public void updateFaces() {
            if (mFaceCount > 1) {
                ...
                Toast.makeText(mActivity.getAndroidContext(), R.string.multiface_crop_help,
    ...
    public void detectFaces(Bitmap bitmap) {
        ...
        float scale = (float) Math.sqrt((double) FACE_PIXEL_COUNT / (width * height));
