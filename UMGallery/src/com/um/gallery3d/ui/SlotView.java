...
import com.um.gallery3d.util.LinkedNode;
...
public class SlotView extends GLView {
    ...
    protected boolean onKeyDown(GLRootView glRootView, KeyEvent event) {
        ...
        if (event.getAction() == KeyEvent.ACTION_UP) {
            return customKeyDetector.onKey(glRootView, event.getKeyCode(), event);
        }
    ...
    private synchronized int renderItem(GLCanvas canvas, ItemEntry entry, float interpolate, int pass,
            boolean paperActive) {
        ...
        if (mPositions != null) {
            ...
            position.x -= mScrollX;
            position.y -= mScrollY;
            Position source = mPositions.getPosition(entry.item.getIdentity(), position);
            source.x += mScrollX;
            source.y += mScrollY;
            position = mTempPosition;
        ...
        if (paperActive) {
            canvas.multiplyMatrix(mPaper.getTransform(position, entry.base, mScrollX, mScrollY), 0);
        } else {
    ...
    public static class Layout {
        ...
        private void initLayoutParameters(int majorLength, int minorLength, /*
                                                                             ...
                                                                             * height
                                                                             */
                int majorUnitSize, int minorUnitSize, /*
                                                       * The slot width and
                                                       * height
                                                       */
                int[] padding) {
            ...
            int usedMinorLength = availableUnits * minorUnitSize + (availableUnits - 1) * mSlotGap;
        ...
        public int getSlotIndexByPosition(float x, float y) {
            ...
            int index = WIDE ? (columnIdx * mUnitCount + rowIdx)
    ...
    public class CustomKeyDetector {
        public boolean onKey(GLRootView glRootView, int keyCode, KeyEvent event) {
            ...
            if (GLView.mCurrrentPhotoSlotIndex != INDEX_NONE && GalleryUtils.isGLRootFocus) {
            ...
            switch (keyCode) {
                ...
                case KeyEvent.KEYCODE_DPAD_UP:
                    ...
                    if (row == 0 && glRootView != null && GalleryUtils.isGLRootFocus) {
                        cancelKeyDown();
                        ...
                        if (offlineButton != null && !GalleryUtils.isFooterContentFocus) {
                ...
                case KeyEvent.KEYCODE_DPAD_CENTER:
                    if (mCurrentFocusSlotIndex > INDEX_NONE) {
                        ...
                        if (exceedScreen() && mCurrentFocusSlotIndex > getScreenSlotCount()) {
                            leftAlignment = false;
                            rightAlignment = true;
                        } else {
                            leftAlignment = true;
    ...
    private int getMoveWidth() {
        ...
        if (WIDE) {
            screenCol = getWidth() / (getSlotWidth() + getmSlotGap());
            noFullSlotWidth = getWidth() - (screenCol * (getSlotWidth() + getmSlotGap()));
            moveWidth = (getSlotWidth() + (getmSlotGap())) - noFullSlotWidth;
        } else {
            screenCol = getHeight() / (getSlotHeight() + getmSlotGap());
            noFullSlotWidth = getHeight() - (screenCol * (getSlotHeight() + getmSlotGap()));
