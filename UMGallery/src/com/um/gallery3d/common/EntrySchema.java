...
public final class EntrySchema {
        ...
        try {
            for (ColumnInfo column : mColumnInfo) {
                ...
                switch (column.type) {
                    case TYPE_STRING:
                        field.set(object,
                                cursor.isNull(columnIndex) ? null : cursor.getString(columnIndex));
                        break;
                    ...
                    case TYPE_BLOB:
                        field.set(object,
                                cursor.isNull(columnIndex) ? null : cursor.getBlob(columnIndex));
                        break;
                }
            }
    ...
    public boolean queryWithId(SQLiteDatabase db, long id, Entry entry) {
        Cursor cursor = db.query(mTableName, mProjection, "_id=?", new String[] {
            Long.toString(id)
        }, null, null, null);
    ...
    public boolean deleteWithId(SQLiteDatabase db, long id) {
        return db.delete(mTableName, "_id=?", new String[] {
            Long.toString(id)
        }) == 1;
    }
    ...
    private void parseColumnInfo(Class<? extends Object> clazz, ArrayList<ColumnInfo> columns) {
        ...
        for (int i = 0; i != fields.length; ++i) {
            ...
            columns.add(new ColumnInfo(info.value(), type, info.indexed(), info.fullText(), info
    ...
    public static final class ColumnInfo {
        ...
        public ColumnInfo(String name, int type, boolean indexed, boolean fullText,
                String defaultValue, Field field, int projectionIndex) {
