...
import com.um.gallery3d.common.Entry.Table;
...
public class FileCache implements Closeable {
    ...
    private static final String QUERY_WHERE = FileEntry.Columns.HASH_CODE + "=? AND "
            + FileEntry.Columns.CONTENT_URL + "=?";
    ...
    private static final String FREESPACE_PROJECTION[] = {
            FileEntry.Columns.ID, FileEntry.Columns.FILENAME, FileEntry.Columns.CONTENT_URL,
            FileEntry.Columns.SIZE
    };
    ...
    public CacheEntry lookup(String downloadUrl) {
        ...
        synchronized (this) {
            ...
            if (!entry.cacheFile.isFile()) { // file has been removed
                try {
                    mDbHelper.getWritableDatabase().delete(TABLE_NAME, ID_WHERE, new String[] {
    ...
    private FileEntry queryDatabase(String downloadUrl) {
        ...
        String whereArgs[] = new String[] {
                String.valueOf(hash), downloadUrl
        };
        Cursor cursor = mDbHelper.getReadableDatabase().query(TABLE_NAME,
                FileEntry.SCHEMA.getProjection(), QUERY_WHERE, whereArgs, null, null, null);
    ...
    private void updateLastAccess(long id) {
        ...
        mDbHelper.getWritableDatabase().update(TABLE_NAME, values, ID_WHERE, new String[] {
            String.valueOf(id)
        });
    }
    ...
    private synchronized void initialize() {
        ...
        Cursor cursor = mDbHelper.getReadableDatabase().query(TABLE_NAME, PROJECTION_SIZE_SUM,
    ...
    private void freeSomeSpaceIfNeed(int maxDeleteFileCount) {
        Cursor cursor = mDbHelper.getReadableDatabase().query(TABLE_NAME, FREESPACE_PROJECTION,
    ...
    private static class FileEntry extends Entry {
        ...
        public String toString() {
            return new StringBuilder().append("hash_code: ").append(hashCode).append(", ")
                    .append("content_url").append(contentUrl).append(", ").append("last_access")
                    .append(lastAccess).append(", ").append("filename").append(filename).toString();
        }
    }
