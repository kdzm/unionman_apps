...
public class FilterType {
    public static void filterTypeImage(Context ctx) {
        SharedPreferences share = ctx.getSharedPreferences("OPERATE", Context.MODE_WORLD_READABLE);
        Editor editor = share.edit();

        // pictrue conditions
        if (share.getBoolean("imageFlag", true)) {
            SharedPreferences.Editor shareImage = ctx.getSharedPreferences("IMAGE",
    ...
    public static void filterTypeMusic(Context ctx) {
        ...
        if (share.getBoolean("audioFlag", true)) {
            SharedPreferences.Editor shareAudio = ctx.getSharedPreferences("AUDIO",
                    Context.MODE_WORLD_READABLE).edit();
