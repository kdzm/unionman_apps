...
import com.um.videoplayer.activity.VideoActivity;
...
import com.um.videoplayer.utility.MD5Tool;
import com.um.videoplayer.utility.TimeTool;
import com.um.videoplayer.utility.ViewTool;
import com.um.videoplayer.utility.base.ImageFileFilter;
...
public class BDActivityNavigation extends ActivityFrame implements OnClickListener {
    ...
    private void setPoster() {
        ...
        String _PosterPath = mModelBDInfo.getPath() + "/BDMV/META/DL";
        File _Dir = new File(_PosterPath);
        String _ImagePath = null;

        // get the largest image file
        if (_Dir.exists() && _Dir.isDirectory()) {
            File[] _Files = _Dir.listFiles(new ImageFileFilter());
            long _Length = 0;

            for (File _File : _Files) {
                if (_File.length() > _Length) {
                    _Length = _File.length();
                    _ImagePath = _File.getAbsolutePath();
                }
            }
        }

        if (_ImagePath != null) {
            mIvPoster.setImageBitmap(BitmapFactory.decodeFile(_ImagePath));
            final String _SrcPath = _ImagePath;
            new Thread() {
                @Override
                public void run() {
                    try {
                        FileTool.cp(_SrcPath, mPosterCachePath);
                    }
