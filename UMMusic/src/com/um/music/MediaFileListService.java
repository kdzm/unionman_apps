...
import com.um.music.Common;
import com.um.music.FMMediaFileList;
import com.um.music.FilterType;
...
public class MediaFileListService extends Service {
    ...
    public MusicModel getMusicInfo(int flag) {
        ...
        if (!((getList() == null) || (getList().size() == 0))) {
            if (flag == 1) {
                ...
                Log.d(TAG," flag == 5 getPreRandomMusicInfo ");
            }else if (flag == 0) {
                // previous Music with no cycle mode
                if (Common.isLoadSuccess()) {
                    if (getCurrPosition() <= 0) {
                        //setCurrPosition(getCurrPosition());
                    	Common.isFirstMediaFile=true;
                    }
                    else {
                        setCurrPosition(getCurrPosition() - 1);
                    }
            ...
            if (Common.isLoadSuccess()) {
                ...
                if(getCurrPosition() == 0)
                {Common.isFirstMediaFile = true;
                }else{ Common.isFirstMediaFile = false;
                }
                if(getCurrPosition() == getList().size() - 1)
                { Common.isLastMediaFile = true;
                }else{ Common.isLastMediaFile = false;
                }
                Log.d(TAG,"Common.isFirstMediaFile = "+Common.isFirstMediaFile+"   Common.isLastMediaFile = "+Common.isLastMediaFile);
            }
            return model;
        }
        else {
    ...
    private boolean isAudioFile(File f) {
        ...
        SharedPreferences shareAudio = getSharedPreferences("AUDIO", Context.MODE_WORLD_READABLE);
    ...
    public void setSignal() {
     ...
     System.out.println(" getCurrPosition() = "+getCurrPosition()+ "  getList().size() = "+ getList().size());
    }
}
