...
public class MediaPlaybackService extends Service {
    ...
    public static boolean isStopedPlayer = false;

    public static final String PLAYSTATE_CHANGED = "com.um.music.playstatechanged";
    public static final String META_CHANGED = "com.um.music.metachanged";
    public static final String QUEUE_CHANGED = "com.um.music.queuechanged";

    public static final String SERVICECMD = "com.um.music.musicservicecommand";
    ...
    public static final String TOGGLEPAUSE_ACTION = "com.um.music.musicservicecommand.togglepause";
    public static final String PAUSE_ACTION = "com.um.music.musicservicecommand.pause";
    public static final String PREVIOUS_ACTION = "com.um.music.musicservicecommand.previous";
    public static final String NEXT_ACTION = "com.um.music.musicservicecommand.next";
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case TRACK_ENDED:
                    ...
                    else if (playMode == SINGLE_PLAYER){
                        pause();
    ...
    public void closeExternalStorageFiles(String storagePath) {
        ...
        if(MediaPlaybackActivity.mediaFileList != null && MediaPlaybackActivity.mediaFileList
     ...
     * "com.um.music.metachanged"
     * "com.um.music.queuechanged",
     * "com.um.music.playbackcomplete"
     * "com.um.music.playstatechanged" respectively indicating
    ...
    public class startLyricThread implements Runnable {
        ...
        public void run() {
            ...
            MediaPlaybackActivity.lyricView.setCurrentPaintColor(Color.BLACK);
            /* MediaPlaybackActivity.lyricView
             .setNotCurrentPaintColor(Color.GREEN);
            MediaPlaybackActivity.lyricView.setCurrentPaintColor(Color.YELLOW);*/
            MediaPlaybackActivity.lyricView.setCurrentTextSize(40);
            MediaPlaybackActivity.lyricView.setLrcTextSize(22);
...
//                .getNextRandomMusicInfo(null).getPath();
        ...
        if (mPlayer.isInitialized()) {
            ...
            status.contentIntent = PendingIntent.getActivity(this, 0,
                                                             new Intent("com.um.music.PLAYBACK_VIEWER")
    ...
    public void prev() {
        synchronized (this) {
            ...
            if (playMode == RANDOM_PLAYER) {
                String filePath  = MediaPlaybackActivity.mediaFileList.getPreRandomMusicInfo(null).getPath();
                  if (null == filePath)
                  { return; }
                  open(filePath);
                  play();
                ...
                return;
            }else if (playMode == CIRCULATE_PLAYER) {
                 String filePath = MediaPlaybackActivity.mediaFileList
    ...
    public void next(boolean force) {
        synchronized (this) {
            ...
            if (playMode == RANDOM_PLAYER) {
                ...
                intent.putExtra("currpath", filePath);
            ...
            else if (playMode == SEQUENCE_PLAYER ||
                     playMode == SINGLE_PLAYER   ||
                     playMode == SINGLE_CIRCULATE){
    ...
    private class MultiPlayer {
        ...
        public void setDataSource(String path) {
            isHiMediaPlayer = true;

            try {
                mHiMediaPlayer.reset();
                mHiMediaPlayer.setOnPreparedListener(null);

                if (path.startsWith("content://")) {
                    mHiMediaPlayer.setDataSource(MediaPlaybackService.this,
                                                 Uri.parse(path));
                    String scheme = Uri.parse(path).getScheme();
                }
                else {
                    mHiMediaPlayer.setDataSource(path);
                }

                // mHiMediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
                mHiMediaPlayer.prepare();
            }
            catch (IOException ex) {
                // TODO: notify the user why the file couldn't be opened
                mIsInitialized = false;
                isHiMediaPlayer = false;
                //return;
            }
            ...
            if (isHiMediaPlayer) {
                mHiMediaPlayer.setOnCompletionListener(listener);
                mHiMediaPlayer.setOnErrorListener(errorListener);
                Intent i = new Intent(
        ...
        HiMediaPlayer.OnCompletionListener listener = new HiMediaPlayer.OnCompletionListener() {
        ...
        HiMediaPlayer.OnErrorListener errorListener = new HiMediaPlayer.OnErrorListener() {
            public boolean onError(HiMediaPlayer mp, int what, int extra) {
                switch (what) {
                        /*
                         * case MediaPlayer.MEDIA_ERROR_SERVER_DIED: mIsInitialized =
                         * false; mHiMediaPlayer.release(); // Creating a new MediaPlayer
                         * and settings its wakemode does not // require the media
                         * service, so it's OK to do this now, while the // service is
                         * still being restarted mHiMediaPlayer = new MediaPlayer();
                         * mHiMediaPlayer.setWakeMode(MediaPlaybackService.this,
                         * PowerManager.PARTIAL_WAKE_LOCK);
                         * mHandler.sendMessageDelayed(mHandler
                         * .obtainMessage(SERVER_DIED), 1000); Log.d(TAG,
                         * "Error: MEDIA_ERROR_SERVER_DIED what=" + what + ", extra=" +
                         * extra +"release MediaPlayer and reset MediaPlayer"); return
                         * true;
                         */
                    default:
                        mIsInitialized = false;
        ...
        private Parcel getInfo(int flag) {
            try {
                ...
                if (isHiMediaPlayer)
                { mHiMediaPlayer.invoke(requestParcel, replayParcel); }
                else
        ...
        public void getMusicInfo() {
            Parcel musicInfo;
            musicInfo = getInfo(CMD_GET_FILE_INFO);

            if (musicInfo != null) {
                int a = musicInfo.readInt();
                musicInfo.readInt();
                int b = musicInfo.readInt();
                int c = musicInfo.readInt();
                int d = musicInfo.readInt();
                musicInfo.readString();
                albumName = musicInfo.readString();
                titleName = musicInfo.readString();
                artistName = musicInfo.readString();
                genreName = musicInfo.readString();
                year = musicInfo.readString();
                date = musicInfo.readString();
            }
        }
