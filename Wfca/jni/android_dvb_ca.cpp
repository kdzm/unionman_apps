...
static jint android_dvb_Wfca_cmd_process(JNIEnv *env, jobject thiz,	jint jcmdType, jbyteArray jareainfoBuff, jintArray jbuffLen)
{
	UM_S32 ret = UM_FAILURE;
	...
	jint *plen = (env)->GetIntArrayElements(jbuffLen,0);
    memcpy(&buff_len, plen, sizeof(int));
	LOGV("android_dvb_wfca_cmd_process jcmdType:%d_xinhua\n", jcmdType);	
	buff = (UM_U8 *)malloc(buff_len);
	if(NULL == buff)
	{
		LOGV("android_dvb_wfca_cmd_process malloc fail_xinhua\n");
		return ret;
	}
    ...
    ret = ca->wfcmdProcess(cmdType,buff, &buff_len);
    if(ret ==  UM_SUCCESS)
    {
    	(env)->SetIntArrayRegion(jbuffLen, 0, 1,(const jint *)&buff_len);
        (env)->SetByteArrayRegion(jareainfoBuff, 0, buff_len,(const jbyte *)buff);
...
//    {"CaGetDvnEntitles",			  "([B[I)I", (void *)android_dvb_Ca_get_dvn_entitles},
	...
	{"WfcaCmdProcess",        "(I[B[I)I", (void *)android_dvb_Wfca_cmd_process},
